...
[dn]
O  = Self-signed TPM root CA for ${hostname}
OU = Self-signed TPM root CA for ${hostname}
CN = Self-signed TPM root CA for ${hostname}
EOF
}
...
function rootca_create () {
    local root_ca_crt=${1}
    local root_ca_privkey=${2}
    local root_ca_config=${3}
    local ecc=${4:-0}
...
rootca_create issuercert.crt signkey.pem root_ca_cert.cfg 
